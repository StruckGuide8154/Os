...
Assert-Match $usermodePath '(FN_BEGIN l3_translate_target[\s\S]*and rax, APP_SLOT_SIZE - 1[\s\S]*cmp rax, \[rel (app_blob_size_v|l3_slot_blob_size)\]|fn l3_translate_target\([\s\S]*\(target - abase\) & SLOT_MASK[\s\S]*bo >= lq\(&(app_blob_size_v|l3_slot_blob_size))' 'L3 target translation must preserve only the installed per-slot blob offset from slot-local callback pointers.'
