...
if ($src -match 'add\s+rdi,\s*L3_HANDLE_TABLE_OFF') {
    throw 'Handle authority regressed to ring-3 slot memory'
}
if ($src -match 'add\s+r11,\s*L3_HANDLE_TABLE_OFF') {
    throw 'Handle authority regressed to ring-3 slot memory'
}
if ($src -match 'USER_ACCESS_BEGIN') {
...
if ($src -notmatch 'test\s+rdx,\s*APP_SLOT_SIZE\s*-\s*1') {
