param(
    ...
    [ValidateSet('Rtl8139', 'VirtioModern', 'VirtioTransitional')]
    [string]$NetworkAdapter = 'Rtl8139',
...
$BootCfg = Join-Path $BUILD 'esp\EFI\BOOT\BOOTCFG.TXT'

if ($NoPassthrough) {
    @(
        '# generated by run_uefi.ps1 -NoPassthrough'
        'usbnic=0'
    ) | Set-Content -Path $BootCfg -Encoding ASCII
    Write-Host "No-passthrough boot config: USB RTL8156 probe disabled; emulated $NetworkAdapter remains enabled." -ForegroundColor Yellow
} elseif (Test-Path $BootCfg) {
    $bootCfgText = Get-Content -Path $BootCfg -Raw -ErrorAction SilentlyContinue
    if ($bootCfgText -like "# generated by run_uefi.ps1 -NoPassthrough*") {
        Remove-Item -Path $BootCfg -Force -ErrorAction SilentlyContinue
    }
}
...
Write-Host "Launching Grit UEFI with XHCI+HID ($PerfProfile, $GuestMemory RAM, net=$NetworkMode/$NetworkAdapter)..." -ForegroundColor Cyan
...
if ($FbWidth -gt 0 -and $FbHeight -gt 0) {
    ...
    $qemuArgs += @('-device', "VGA,vgamem_mb=32,xres=$FbWidth,yres=$FbHeight,edid=on")
} else {
    # Use an explicit VGA device instead of '-vga std' so the GOP mode OVMF
    # advertises has enough backing memory for WUXGA/32bpp full-frame presents.
    $qemuArgs += @('-device', 'VGA,vgamem_mb=32')
}
if (-not $MscTest) {
...
if ($NoEmulatedNic) {
    ...
    $netdev = if ($NetworkMode -eq 'Tap') { "tap,id=net0,ifname=$TapIfName" } else { 'user,id=net0' }
    $nic = switch ($NetworkAdapter) {
        'VirtioModern'       { 'virtio-net-pci,netdev=net0' }
        'VirtioTransitional' { 'virtio-net-pci,netdev=net0,disable-modern=on' }
        default              { 'rtl8139,netdev=net0' }
    }
    $qemuArgs += @('-netdev', $netdev, '-device', $nic)
}
